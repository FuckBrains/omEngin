...
class RPA:
    ...
    def add_cond(self, k, v):
        ...
        self.dic = ndc2
    def apply_filter(self, OnColumn='CAT', ls_values = ['NA'], operation = 'keep'):
        print(self.df)
        if operation == 'keep':
            self.df = self.df[~self.df[OnColumn].isin(ls_values)]
        else:
            self.df = self.df[self.df[OnColumn].isin(ls_values)]
        print(self.df)
    ...
    def rpagen(self):
        ...
        print(hpf, chr(10))
    def rpagen_count(self,thread_column='sZone',ls_colval=False):
        hpf = chr(10)
        ndf = self.df
        print(ndf.columns)
        th = ''
        if ls_colval == False:
            th = ndf[thread_column].unique().to_list()
        else:
            th = ls_colval
        hd = ''
        for j in th:
            ...
            for i in range(len(self.lsx)):
                ...
                if len(self.lsky) == len(spl):
                    ...
                    for n in range(len(spl)):
                        ...
                        xxdf = xdf[xdf[thread_column].isin([j])]
                        if hd == '':
                            hd = hpp
                        else:
                            if hpp not in hd:
                                hd = hd + '/' + hpp
                        if hp == '':
                            hp = j + ':' + str(xxdf.shape[0])
                        else:
                            hp = hp + '/' + str(xxdf.shape[0])
            else:
                hpf = hpf + chr(10) + hp
        else:
            print(thread_column,':',hd)
            print(hpf)
